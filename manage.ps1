...
switch ($args[0]) {
    "build" {
        ...
        javac -sourcepath $SourceDir -d $OutputDir "$SourceDir/Main.java" "$SourceDir/Controllers/*.java" "$SourceDir/Devices/*.java"
    ...
    "run" {
        if (Test-Path $OutputDir) {
            ...
            # O Main continua sendo executado da raiz da pasta 'out'
